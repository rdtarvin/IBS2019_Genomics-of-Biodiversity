...
Oops! What happened? Let's search the error. <br>
<details> 
  <summary>How can we fix this run?</summary>
...
[or we can do a short exercise here pati]<br>
	...
	# --inline_index tells the program that the first barcode is inline and the second is in the index (header of the read)
	# actually, these data have inline_inline barcodes, but if you recall we removed the second inline barcode with fastx-toolkit
	
	# --renz_1 is the first enzyme (cut site present in R1)
	# --renz_2 is the second enzyme (cut site present in R2)
	# -i indicates input format
	# --disable_rad_check tells the program to keep reads that are missing barcodes (recall we removed R2 barcodes...)
