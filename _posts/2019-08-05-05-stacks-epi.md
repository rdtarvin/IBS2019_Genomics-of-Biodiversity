...
Demultiplexing your sequencing pools is always the first step in any pipeline, before you even begin the genotyping or assembl process. In stacks, the files you need for demultiplexing are: 
...
STACKS is often used for population genetics, but our dataset was designed for phylogenetics, so here we will consider each sample to be its own population.<br>

Let's start setting up denovo_map runs. First, move the file that have good sequences in them into a new `good` folder using the following commands. We'll
...
The `good` directory will be a part of the input for running *denovo_map.pl*, since you have to list all of the sequence files that will be used for input, rather than a directory containing them (and you don't want to include the discarded sequences!). 
...
> Let's take a look at some parameter tests that we recently did with our MingaGenomica held at PUCE this past spring! 
> 
